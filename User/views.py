...
def send_mail_after_registration(request, email, username, token):
    ...
    If settings.PUBLIC_SITE_URL is set, use that as the base for the verification
    link so it always points to the correct public URL (domain or LAN IP),
        ...
        if password_form.is_valid():
            ...
            if user_email.exists():
                ...
                for user in user_email:
                    subject = "Password Reset - Sachris Club"
                    email_template_name = 'registration/password_reset_email-1.html'
                    from django.urls import reverse
                    uid = urlsafe_base64_encode(force_bytes(user.pk))
                    token = default_token_generator.make_token(user)
                    reset_path = reverse('password_reset_confirmm', kwargs={'uidb64': uid, 'token': token})
    ...
    if 'userid' in request.session:
        if request.method == 'POST':
            ...
            if int(total_quantity) <= int(suma):
                if payment_option == 'COD' or payment_option == 'QR':
                    buy_obj = buyModel()
                ...
                if cart_product:
                    ...
                    if int(len(main_total_quantity)) == int(len(qun)):
                        ...
                        send_data = {'click_on': '1', 'cart_val': count, 'cartt': alldata, 'total_price': total_price,
                                     ...
                                     'subtotal': summ, 'var_11': 'varr', 'upi_id': getattr(settings, 'UPI_ID', '')}
